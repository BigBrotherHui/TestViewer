...
DRRgenerator::DRRgenerator()
{
    ...
    float roll=/*-M_PI/2*/-M_PI/6; // rotation around x-axis
    float pitch=/*M_PI/2*/0; // rotation around y-axis
    float yaw=/*-M_PI/2*/0; // rotation around z-axis
    ...
    cam.intrinsics_video.at<double>(0,2)=320;//离图像原点的坐标，此时假设当前focal穿过的是视平面中心，所以大小是输出图像大小的一半
 ...
 */
//用于计算X射线通过物质时的衰减系数 attenuation->衰减
//X射线在物质中传播时，会因为与物质相互作用而衰减，这个衰减是由物质的线性衰减系数决定的。
//
//这里将水的线性衰减系数作为一个基准值，加上一个水的线性衰减系数是为了确保即使像素密度较小的情况下（低于 min_hu），也能有一个小的衰减系数，以充分模拟物质的吸收效应。这个处理方式可能是为了更好地适应不同密度物质的情况，确保在计算衰减系数时不会出现严重的断裂或失真。
