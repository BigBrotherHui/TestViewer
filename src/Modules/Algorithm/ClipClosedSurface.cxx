...
vtkStandardNewMacro(ClipClosedSurface);

vtkCxxSetObjectMacro(ClipClosedSurface, ClippingPlanes, vtkPlaneCollection);
...
//------------------------------------------------------------------------------
const char* ClipClosedSurface::GetScalarModeAsString()
{
...
//------------------------------------------------------------------------------
void ClipClosedSurface::PrintSelf(ostream& os, vtkIndent indent)
{
...
//------------------------------------------------------------------------------
int ClipClosedSurface::ComputePipelineMTime(vtkInformation* vtkNotUsed(request),
...
void ClipClosedSurface::SetClipPlaneColor(int id, unsigned char* color) { 
...
vtkSmartPointer<vtkPolyData> ClipClosedSurface::GetClippingPlanePolyData() {
  return tmpContourData;
}
  //------------------------------------------------------------------------------
int ClipClosedSurface::RequestData(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  tmpContourData = vtkSmartPointer<vtkPolyData>::New();
  ...
  if (this->ScalarMode == VTK_CCS_SCALAR_MODE_COLORS)
  {
    ...
    ClipClosedSurface::CreateColorValues(
  ...
  {
    ClipClosedSurface::BreakPolylines(
  ...
  for (int planeId = 0; planes && (plane = planes->GetNextPlane(iter)); planeId++)
  {
    ...
    if (polys)
    {
      ...
      if (scalars)
      {
        unsigned char color[3]/* = colors[1 + active]*/;
        for (int i = 0; i < 3; ++i) {
          color[i] = Colors[planeId][i];
        }
  ...
  ClipClosedSurface::SqueezeOutputPoints(output, points, pointData, inputPointsType);
  output->Squeeze();
...
//------------------------------------------------------------------------------
void ClipClosedSurface::SqueezeOutputPoints(
...
//------------------------------------------------------------------------------
void ClipClosedSurface::CreateColorValues(const double color1[3], const double color2[3],
...
int ClipClosedSurface::InterpolateEdge(vtkPoints* points, vtkPointData* pointData,
...
//------------------------------------------------------------------------------
void ClipClosedSurface::ClipLines(vtkPoints* points, vtkDoubleArray* pointScalars,
  ...
  for (vtkIdType cellId = 0; cellId < numCells; cellId++)
  {
    ...
    for (vtkIdType i = 1; i < numPts; i++)
    {
      ...
      if ((c0 | c1))
      {
        ...
        if ((c0 ^ c1))
        {
          ClipClosedSurface::InterpolateEdge(
...
//------------------------------------------------------------------------------
void ClipClosedSurface::ClipAndContourPolys(vtkPoints* points, vtkDoubleArray* pointScalars,
...
//------------------------------------------------------------------------------
void ClipClosedSurface::BreakPolylines(vtkCellArray* inputLines, vtkCellArray* lines,
...
//------------------------------------------------------------------------------
void ClipClosedSurface::CopyPolygons(vtkCellArray* inputPolys, vtkCellArray* polys,
...
//------------------------------------------------------------------------------
void ClipClosedSurface::BreakTriangleStrips(vtkCellArray* inputStrips, vtkCellArray* polys,
...
//------------------------------------------------------------------------------
void ClipClosedSurface::TriangulateContours(vtkPolyData* data, vtkIdType firstLine,
...
// ---------------------------------------------------
int ClipClosedSurface::TriangulatePolygon(
