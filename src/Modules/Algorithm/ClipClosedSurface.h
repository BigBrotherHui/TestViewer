...
#ifndef ClipClosedSurface_h
#define ClipClosedSurface_h
...
class Algorithm_EXPORT ClipClosedSurface : public vtkPolyDataAlgorithm {
public:
  static ClipClosedSurface* New();
  vtkTypeMacro(ClipClosedSurface, vtkPolyDataAlgorithm);
  void PrintSelf(ostream& os, vtkIndent indent) override;
  void SetClipPlaneColor(int id, unsigned char* color);
  ///@{
  /**
  ...
  ///@}
  vtkSmartPointer<vtkPolyData> GetClippingPlanePolyData();
  ///@{
  /**
...
protected:
  ClipClosedSurface();
  ~ClipClosedSurface() override;
  vtkSmartPointer<vtkPolyData> tmpContourData;
  vtkPlaneCollection* ClippingPlanes;
  std::map<int,std::array<unsigned char, 3>> Colors;
  double Tolerance;
...
private:
  ClipClosedSurface(const ClipClosedSurface&) = delete;
  void operator=(const ClipClosedSurface&) = delete;
};
