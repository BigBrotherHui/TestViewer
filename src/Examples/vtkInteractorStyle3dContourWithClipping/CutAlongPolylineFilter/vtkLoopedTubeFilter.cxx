	...
	vtkInformationVector* outputVector)
{
	...
	std::cout << "(((" << input->GetCell(0)->GetNumberOfPoints() << "(((" << input->GetNumberOfPoints() << std::endl;
	if (/*input->GetCell(0)->GetNumberOfPoints() != input->GetNumberOfPoints() + 1 ||*/
		input->GetCell(0)->GetPointId(input->GetCell(0)->GetNumberOfPoints()-1) != 0) {
		for(int i=0;i<input->GetCell(0)->GetNumberOfPoints();++i)
		{
			std::cout << i << " " << input->GetCell(0)->GetPointId(i) << std::endl;
		}
...
void vtkLoopedTubeFilter::GenerateStripsForLoop(vtkIdType offset, vtkIdType npts,const vtkIdType* vtkNotUsed(pts), vtkIdType inCellId, vtkCellData *cd, vtkCellData *outCD, vtkCellArray *newStrips)
{
...
int vtkLoopedTubeFilter::GeneratePointsForLoop(vtkIdType offset, vtkIdType npts,const vtkIdType *pts, vtkPoints *inPts, vtkPoints *newPts, vtkPointData *pd, vtkPointData *outPD, vtkFloatArray *newNormals, vtkDataArray *inScalars, double range[2], vtkDataArray *inVectors, double maxSpeed, vtkDataArray *inNormals)
{
	vtkIdType j;
	int i, k;
	double p[3];
