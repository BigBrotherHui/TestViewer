...
vtksplinewidget::vtksplinewidget()
{
  ...
  this->HandleGeometry = new vtkRegularPolygonSource*[this->NumberOfHandles];
  //int i;
  //double u[3];
  //double x0 = bounds[0];
  //double x1 = bounds[1];
  ...
  {
    this->HandleGeometry[i] = vtkRegularPolygonSource::New();
    this->HandleGeometry[i]->SetNumberOfSides(70);
    this->HandleGeometry[i]->SetRadius(1);
    ...
    this->HandleGeometry[i]->SetCenter(0,0,0);
  }
...
{
  if (this->State == vtksplinewidget::Outside || this->State == vtksplinewidget::Start)
  {
    return;
  }
